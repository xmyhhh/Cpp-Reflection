...
ClangParser::ClangParser(std::string _target_parse_json_file, std::string _temp_folder,
                         std::string _clang_include_folder) :
...
int ClangParser::Parse()
{
    ...
    std::vector<const char*> clang_arg = {
        {
            "-x",
            "c++",
            "-std=c++11",
            ...
            case CXCursor_ClassDecl: //如果找到class定义
                {
                    ...
                    if (ClangCursorUtils::CursorAnnotateAttrCamp(current_cursor, ReflectionProperty::Class_AnnotateAttr
                                                                 ))
                    {
                        //如果该class有AnnotateAttr，并且是class AnnotateAttr
                        std::cerr << "find class: " << ClangCursorUtils::GetCXCursorName(current_cursor) << std::endl;
                        ClangParser* p = (ClangParser*)client_data;
                        p->m_class.push_back(std::make_unique<RClass>(current_cursor));
                    }
                    else
                    {
                    }
...
{
    RefelactionGenerator generator(temp_folder_path);
    generator.Generate(m_class);
    return 0;
}
